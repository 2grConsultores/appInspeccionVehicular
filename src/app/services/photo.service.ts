...
export class PhotoService {
  ...
  public async takePhoto_to_url(path:string) {
    try {
      ...
      const url = await this.uploadImage(blob, image, path);
  ...
  async uploadImage(blob: any, imageData: any, path:string) { 
    try {
      const currentDate = Date.now();
      const filePath = `${path}/${currentDate}.${imageData.format}`;
